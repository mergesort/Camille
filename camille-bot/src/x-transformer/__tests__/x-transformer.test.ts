...
describe('X/Twitter Transformer Module', () => {
  ...
  describe('transformXToXcancel', () => {
    ...
    test('should preserve paths when transforming', () => {
      expect(transformXToXcancel('x.com/profile/mergesort')).toBe(
        'https://xcancel.com/profile/mergesort'
      );
      expect(transformXToXcancel('twitter.com/profile/mergesort')).toBe(
        'https://xcancel.com/profile/mergesort'
      );
      expect(transformXToXcancel('https://x.com/mergesort/status/594915993060777984')).toBe(
        'https://xcancel.com/mergesort/status/594915993060777984'
      );
    });
    ...
    test('should handle query parameters and hash fragments', () => {
      ...
      expect(transformXToXcancel('twitter.com/profile/mergesort?lang=en')).toBe(
        'https://xcancel.com/profile/mergesort?lang=en'
      );
      expect(transformXToXcancel('https://x.com/search?q=test#results')).toBe(
        'https://xcancel.com/search?q=test#results'
      );
    });

    test('should handle Slack formatting', () => {
      expect(transformXToXcancel('<https://x.com/profile/mergesort>')).toBe(
        'https://xcancel.com/profile/mergesort'
      );
      expect(transformXToXcancel('<https://twitter.com/profile/mergesort|Twitter Profile>')).toBe(
        'https://xcancel.com/profile/mergesort'
      );
    });
  ...
  describe('processXLinks', () => {
    test('should detect and transform a single X/Twitter link', async () => {
      const result = await processXLinks(
        'Check out this post: https://x.com/mergesort/status/594915993060777984',
        mockLogger
      );
      ...
      expect(result.transformedLinks[0]).toBe(
        'https://xcancel.com/mergesort/status/594915993060777984'
      );
    ...
    test('should handle errors gracefully', async () => {
      ...
      const result = await processXLinks('Check out https://x.com/test', mockLogger);
    ...
    test('should properly deduplicate identical URLs', async () => {
      ...
      const result = await processXLinks(message, mockLogger);
  ...
  describe('Regex testing', () => {
    ...
    test('should not match domains that contain x.com or twitter.com as substrings', () => {
      ...
      const message =
        'Check these: https://netflix.com and also https://phoenix.com and mytwitter.com';
    ...
    test('should match x.com and twitter.com with subdirectories', () => {
      ...
      const message =
        'Check these: https://x.com/profile/123 and also https://twitter.com/username/status/456';
